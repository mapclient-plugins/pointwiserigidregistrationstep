...
class PointWiseRigidRegistrationStep(WorkflowStepMountPoint):
    ...
    def __init__(self, location):
        ...
        self._config = {
            'identifier': '',
            'UI Mode': True,
            'Registration Method': 'Correspondent Affine',
            'Min Relative Error': '1e-3',
            'Points to Sample': '1000',
            'Init Trans': [0.0, 0.0, 0.0],
            'Init Rot': [0.0, 0.0, 0.0],
            'Init Scale': 1.0
        }
    ...
    def _manual_transform(self):
        ...
        transform = np.hstack([t0, r0, s0])
        self.sourceDataAligned = transform3D.transformRigidScale3DAboutCoM(self.sourceData, transform)
        self.transform = regMethodTransforms[self._config['Registration Method']](transform)
    ...
    def _register(self):
        ...
        print('T0:', x0)
        if x0 is None:
            transform, self.sourceDataAligned, (rmse0, self.RMSE) = reg(
                self.sourceData, self.targetData, xtol=xtol, sample=samples, output_errors=True)
        else:
            transform, self.sourceDataAligned, (rmse0, self.RMSE) = reg(
                self.sourceData, self.targetData, t0=x0, xtol=xtol, sample=samples, output_errors=True)
    ...
    def _parse_legacy_params(self):
        ...
        if isinstance(self._config['Init Trans'], str):
            self._config['Init Trans'] = _parse_str_list(self._config['Init Trans'])

        if isinstance(self._config['Init Rot'], str):
            self._config['Init Rot'] = _parse_str_list(self._config['Init Rot'])
